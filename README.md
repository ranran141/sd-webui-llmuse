...
LM Studio のローカル LLM を使って、日本語入力から Stable Diffusion 用英語プロンプトを生成する SD WebUI Forge 拡張機能です
...
- 使用するモデルをドロップダウンで切り替え（切り替え時に前のモデルを自動アンロード）
- Auto-unload で使用後に VRAM を自動解放
- Auto-send で生成後にプロンプト欄へ自動送信

## インストール方法

**LM Studio のセットアップ（先に行ってください）**
...
3. **Developer** タブで **Start Server** をクリックし、`http://localhost:1234` でサーバーを起動
...
4. **Send** をクリックして Forge のプロンプト欄に追記
